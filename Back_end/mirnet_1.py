...
def find_a_place_for_trained_model(enhancer="mirnet"):
...
def train_model ( IN_COLAB):
    ...
    if not IN_COLAB:
        ...
        base_dir = "/content/gdrive/MyDrive/Colab Notebooks/mirnet_training_data_spectrograms"

        train_low_light_images = sorted(glob(base_dir + "/our485/low/*"))[:MAX_TRAIN_IMAGES]
        train_enhanced_images = sorted(glob(base_dir + "/our485/high/*"))[:MAX_TRAIN_IMAGES]

        val_low_light_images = sorted(glob(base_dir + "/our485/low/*"))[MAX_TRAIN_IMAGES:]
        val_enhanced_images = sorted(glob(base_dir + "/our485/high/*"))[MAX_TRAIN_IMAGES:]

        test_low_light_images = sorted(glob(base_dir + "/eval15/low/*"))
        test_enhanced_images = sorted(glob(base_dir + "/eval15/high/*"))
    ...
    filepath = find_a_place_for_trained_model("mirnet")
...
def make_predictions (IN_COLAB, test_low_light_images):
    from tensorflow.keras import models

    print (f"making predictions")

    filepath = find_a_place_for_trained_model("mirnet")
    model = models.load_model(model, filepath)




    for random.sample(test_low_light_images, 6):
        original_image = Image.open(low_light_image)
        enhanced_image = infer(model, original_image)
        plot_results(
